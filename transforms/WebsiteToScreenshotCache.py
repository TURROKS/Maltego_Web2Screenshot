...
def api_handler(api_url, target_url, api, custom_agent):
    ...
    data = requests.get(api_url+f"?access_key={api}&url=https://{target_url}&user_agent={custom_agent}")
...
@registry.register_transform(display_name="To Screenshot - Cache [API Flash]", input_entity="maltego.Website",
                             description='Take a cache screenshot from an Website, will consume 1 credit if not '
                                         'available in cache.',
                             output_entities=["maltego.Image"])
class WebsiteToScreenshotCache(DiscoverableTransform):
